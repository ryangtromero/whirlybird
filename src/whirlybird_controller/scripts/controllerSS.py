...
class Controller():
    ...
    def whirlybirdCallback(self, msg):
        ...
        C_lon = concatenate((B_lon, A_lon*B_lon),axis=1)
        ...
        C_yaw = matrix([0, 1, 0, 0])

        K_lat = place(A_lat, B_lat, p_lat)
        K_r_lat = -1/(Cr_lat*linalg.inv(A_lat - B_lat*K_lat)*B_lat)
        ...
        K_long = place(A_lon, B_lon, p_lon)
        K_r_long = -1/(Cr_lon*linalg.inv(A_lon - B_lon*K_long)*B_lon)
        ##################################
        ...
        F = self.theta_r*K_r_long - K_long*x_long + Fe

        tau = self.psi_r*K_r_lat - K_lat*x_lat
